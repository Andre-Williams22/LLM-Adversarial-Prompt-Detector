...
from utils.mlflow_setup import EXPERIMENT_NAME, configure_mlflow
...
class FastAdversarialDetector:
    ...
    # Every threshold must be monotonically ordered high <= balanced <=
    # conservative, so that raising the sensitivity can only ever add
    # detections. tests/test_hybrid_voting.py enforces that invariant.
    VOTING_PRESETS = {
        "high": {
            "high_confidence_threshold": 0.40,
            "toxic_confidence_threshold": 0.50,
            "hate_confidence_threshold": 0.40,
            "weak_signals_threshold": 0.15,
            "weak_signals_count": 2,
            "majority_threshold": 0.25,
            "weighted_threshold": 0.20,
            "keyword_weight": 0.40,
            "other_weights": [0.25, 0.20, 0.15],
        },
        "balanced": {
            "high_confidence_threshold": 0.50,
            "toxic_confidence_threshold": 0.60,
            "hate_confidence_threshold": 0.50,
            "weak_signals_threshold": 0.20,
            "weak_signals_count": 2,
        ...
        "conservative": {
            ...
            "toxic_confidence_threshold": 0.80,
            "hate_confidence_threshold": 0.70,
            "weak_signals_threshold": 0.40,
            "weak_signals_count": 3,
    ...
    # A keyword score above this short-circuits the three transformer stages.
    # It sits below the balanced high-confidence threshold on purpose: a clear
    # keyword hit is decisive enough on its own to skip the ~300ms of model
    # inference that would only confirm it.
    EARLY_EXIT_THRESHOLD = 0.45

    def _configure_voting_sensitivity(self) -> Dict:
        """Return the voting thresholds for the active sensitivity mode.

        An unrecognised mode falls back to balanced rather than raising, so a
        bad environment variable degrades to the default policy instead of
        taking the service down.
        """
        preset = self.VOTING_PRESETS.get(self.sensitivity_mode)
        if preset is None:
            logger.warning(
                "Unknown sensitivity mode %r, falling back to %r",
                self.sensitivity_mode, self.DEFAULT_SENSITIVITY,
            )
            preset = self.VOTING_PRESETS[self.DEFAULT_SENSITIVITY]
        return dict(preset)

    def setup_mlflow(self):
        """Point MLflow at the shared experiment. Best effort: tracking is
        observability, so a tracking outage must not stop classification.

        The result is cached on the instance so that an unreachable tracking
        store costs one warning at startup rather than five per request.
        """
        self.mlflow_enabled = configure_mlflow(EXPERIMENT_NAME)
    ...
    def load_fast_models(self) -> Dict:
        ...
        logger.debug("Loading toxic classifier")
        ...
        logger.debug("Loading hate speech classifier")
        ...
        logger.debug("Loading keyword detector")
        models['keyword_detector'] = self.create_keyword_detector()

        logger.info("Loaded %d detection models", len(models))
        return models
    ...
    def detect_adversarial_sync(self, text: str) -> Tuple[bool, dict]:
        ...
        self._log_model_run(
            ...
            is_adversarial=keyword_score > self.EARLY_EXIT_THRESHOLD
        )

        # Early exit for obvious cases
        if keyword_score > self.EARLY_EXIT_THRESHOLD:
            total_time = time.time() - start_time

            logger.info("Early exit: keyword score %.4f exceeds %.2f",
                        keyword_score, self.EARLY_EXIT_THRESHOLD)
    ...
    def toxic_check(self, text: str) -> float:
        ...
        try:
            ...
            logger.warning("Toxic check failed: %s", e)
            return 0.0
    ...
    def hate_check(self, text: str) -> float:
        ...
        try:
            ...
            logger.warning("Hate check failed: %s", e)
            return 0.0
    ...
    def hybrid_voting_decision(self, scores: List[float]) -> Tuple[bool, str, dict]:
        ...
        if (keyword_score > config["high_confidence_threshold"]
                or toxic_score > config["toxic_confidence_threshold"]
                or hate_score > config["hate_confidence_threshold"]
                or safety_score > config["high_confidence_threshold"]):
    ...
    def _log_model_run(self, model_name: str, score: float, inference_time: float,
        ...
        try:
            ...
            logger.warning("Failed to log %s run: %s", model_name, e)
    ...
    def _log_ensemble_decision(self, scores: list, final_decision: bool, reason: str,
        ...
        try:
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                logger.info(ensemble_log)

        except Exception as e:
            logger.warning("Failed to log ensemble decision: %s", e)


async def detect_adversarial_prompt_fast(text: str, sensitivity_mode: str = None) -> Tuple[bool, dict]:
    """Classify a prompt using the shared detector instance.

    The instance is owned by utils.detector_registry rather than by this
    module, so importing the detector class does not download model weights as
    a side effect.

    Args:
        text: Input text to analyze.
        sensitivity_mode: Optional policy preset override ("high", "balanced",
            "conservative"). Switching reloads the models, so this is a
            control-plane operation, not a per-request one.
    """
    from utils.detector_registry import registry

    if sensitivity_mode and sensitivity_mode != registry.sensitivity_mode:
        detector = registry.set_sensitivity(sensitivity_mode)
    else:
        detector = registry.get()

    return await detector.detect_adversarial_fast(text)
